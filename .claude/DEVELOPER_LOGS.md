...
## 2026-09-26 — The robot voice goes, the parent's voice comes in

The parent asked for every spoken prompt to be configurable, with an uploaded file for each "instead of using the cheesy text to speech". Two choices shaped the build, both theirs. First, a line without a recording is **silent**, not robot-voiced, so the synthesizer is gone completely: `speak()`, `sayOrPlay()` and the speech warm-up in the iOS audio unlock are deleted, and an E2E test now fails if anything ever calls `speechSynthesis.speak`. Second, recordings are **per hunt**, stored on the hunt as `voiceLines` and carried over when a hunt is duplicated. (Clue photos and voices are cleared on duplicate because they belong to hiding places. These don't.)

`lib/voiceLines.ts` is the catalogue: 16 lines, each with its words and a plain note of when it plays. They are the six puzzle instructions, two celebrations, "Go find it!" and "Go find the treasure!", two "Not yet!" lines and four Counting Lock nudges. The lock's hint used to be generated ("Dial 2 is more than 5!"), which can't be pre-recorded. So it's now spoken as just "It's a bigger number!" or "It's a smaller number!", while the bubble still shows the exact dial and number.

On the play side, the station response carries the hunt's recordings (only when there are any, so a hunt without them sends exactly what it did before). A small React context hands them to every screen and puzzle, including the marble run's portalled free build, without threading a new prop through the puzzle contract. Speaker buttons only render when there is something to play. The "Not yet" screen's "Say it" button, which existed only to trigger the robot, became a speaker beside the words.

On the setup side, a folding "Voice lines" panel lists every line with a tick or a muted icon and a running "3 of 16 added". The existing recorder gained an **Upload file** button, so clue voices and lock questions can be uploaded too. Browsers label audio files inconsistently (.m4a arrives as `audio/x-m4a`, `audio/mp4` or nothing at all), so the extension decides the type and the file is relabelled before upload. The server now also accepts WAV and the MP3 variants phones produce.

## 2026-09-26 — A big red button to begin

The parent wanted a start screen before the first scan: "just a big red button" that plays their message and then opens the scanner. They chose to make it the front page itself, whenever a hunt is live, so starting the day is just opening the site on the iPad.

"Which hunt is live?" needed a real answer, because more than one can be. Hunts now get an `activatedAt` stamp when they go live, set in `applySave` from the server's clock, never the client's, and kept through later edits. `currentLiveHunt()` picks the most recently activated one, falling back to the creation date for hunts that went live before the stamp existed. The front page calls `connection()` so Next renders it per request rather than freezing whichever hunt was live at build time. It hands the browser nothing but the welcome recording's address. If the store can't be read, it falls back to the old "scan a code" page rather than failing.
